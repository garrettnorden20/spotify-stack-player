...
def get_device_id(sp: Spotify) -> str:
    ...
    if not devices:
        raise RuntimeError("No active Spotify devices found")
    return devices[0]["id"]
...
def toggle_playback(sp: Spotify):
    ...
    try:
        ...
        if controller.paused_time is None:
            ...
            if (
                controller.stepped_uri is None
            ):  # not in album, so resume from our list of queued
                sp.start_playback(
                    controller.device_id,
                    None,
                    controller.queued_song_uris,
                    {"uri": controller.paused_uri},
                    controller.paused_time,
                )
            else:  # resume from within our album
                sp.start_playback(
                    controller.device_id,
                    controller.album_uri,
                    None,
                    {"uri": controller.paused_uri},
                    controller.paused_time,
                )
...
def queue_new(sp):
    ...
    try:
        ...
        if (
            controller.stepped_uri is not None
            and controller.queued_song_uris is not None
            and len(controller.queued_song_uris) > 0
        ):
            print("Stepping out")
            sp.start_playback(
                controller.device_id,
...
def get_all_top_tracks(sp, max_tracks=50, batch_size=50):
    ...
    while max_tracks is None or len(tracks) < max_tracks:
        items = sp.current_user_top_tracks(limit=batch_size, offset=offset).get(
            "items", []
        )
        if not items:
            break
...
def main():
    ...
    controller.device_id = get_device_id(sp)
